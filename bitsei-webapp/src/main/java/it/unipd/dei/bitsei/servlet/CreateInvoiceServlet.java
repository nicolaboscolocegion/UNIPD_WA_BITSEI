...
import it.unipd.dei.bitsei.dao.CreateInvoiceDAO;
 ...
 * @since 1.00
 */
public final class CreateInvoiceServlet extends AbstractDatabaseServlet {
    /**
     * Creates a new invoice into the database.
     *
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res)
        ...
        try {
            ...
            i = new Invoice(customer_id, status, warning_number, warning_date, warning_pdf_file, invoice_number, invoice_date, invoice_pdf_file, invoice_xml_file, total, discount, pension_fund_refund, has_stamp);

            // creates a new object for accessing the database and stores the invoice
            new CreateInvoiceDAO(getConnection(), i).access();

            m = new Message(String.format("Invoice successfully stored in the database."));
            ...
            if ("23505".equals(ex.getSQLState())) {
                m = new Message(String.format("Cannot create the invoice: invoice already exists."), "E300",
                        ex.getMessage());

                LOGGER.error(
                        new StringFormattedMessage("Cannot create the invoice: invoice already exists."),
                        ex);
            } else {
                m = new Message("Cannot create the invoice: unexpected error while accessing the database.", "E200",
                        ex.getMessage());

                LOGGER.error("Cannot create the invoice: unexpected error while accessing the database.", ex);
            }
